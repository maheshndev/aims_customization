...
frappe.ui.form.on("Sales Invoice", {
    refresh(frm) {
        if (!frm.doc.__islocal) {
            ...
            // Condition 1: Normal Sales Invoice
            if (!frm.doc.is_return) {
                allowed_pf = "Sales Invoice Aims";
            }

            // Condition 2: Credit Note (Sales Invoice Return)
            if (frm.doc.is_return == 1) {
                allowed_pf = "Credit Note Aims";
            }
            ...
            setTimeout(() => {
                Object.keys(frappe.boot.print_formats).forEach(function (pf_name) {
                    if (
                        pf_name !== allowed_pf &&
                        frappe.boot.print_formats[pf_name].doc_type === "Sales Invoice"
                    ) {
                        frappe.boot.print_formats[pf_name].hidden = 1;
                    }
                });
