<template>
	<div class="bom-list rounded-xl shadow-sm p-2 bg-white border border-gray-100">
		<p class="text-xs text-gray-500">
			BOM For calculate then capacity Using Cavity , Cycle Time, Shift, Workstation/Machine,
			Mould select mould and workstation/machine from dropdown for calculate capacity and
			schedule work orders
		</p>
		<div v-if="loading" class="text-gray-500 animate-pulse p-4 text-center">
			Loading BOMs...
		</div>
		...
		<div v-if="!loading" class="flex justify-between items-center mb-3">
			<div class="flex gap-2">
				<button
					class="px-2 py-1 bg-blue-50 text-blue-700 font-medium rounded hover:bg-blue-100 transition shadow-sm"
					@click="selectAll"
				>
					Select All
				</button>
				<button
					class="px-2 py-1 bg-gray-50 text-gray-700 font-medium rounded hover:bg-gray-100 transition shadow-sm"
					@click="unselectAll"
				>
					Unselect All
				</button>
		...
		<div
			v-if="filteredBOMs.length && !loading"
			class="overflow-auto rounded-xl border border-gray-200 shadow-sm"
		>
			<table class="min-w-full table-auto divide-y divide-gray-200 text-sm">
				...
				<tbody class="divide-y divide-gray-200 bg-white">
					<tr
						v-for="(bom, index) in filteredBOMs"
						:key="bom.row_uid"
					...
					>
						...
						<td class="px-2 py-2 border text-gray-500 whitespace-nowrap">
							{{ index + 1 }}
						</td>
						...
						<td class="px-2 py-2 border font-medium whitespace-nowrap">
							<span
								...
								@click="openBOM(bom.bom_no)"
								>{{ bom.bom_no || "No BOM Available" }}</span
							>
						</td>
						<td
						...
						<td class="px-2 py-2 border font-bold text-orange-600 whitespace-nowrap">
							{{ bom.cavity }}
						</td>
						...
						<td class="px-2 py-2 border min-w-[180px] whitespace-nowrap">
							<select
								v-model="bom.selected_workstation"
							...
							>
								...
								<option
									v-for="op in bom.bom_operations"
									:key="op.workstation"
						...
						<td class="px-2 py-2 min-w-[200px] whitespace-nowrap">
							<select
								v-model="bom.selected_mould"
								class="w-full text-xs border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500"
							>
								<option disabled value="">Select Mould</option>
								<option
									v-for="mould in bom.moulds"
									:key="mould.mould_no"
									:value="mould.mould_no"
								>
