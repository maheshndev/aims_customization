  ...
  <div class="bom-list rounded-xl shadow-sm p-2 bg-white border border-gray-100">
    ...
    <div v-if="filteredBOMs.length && !loading" class="flex justify-between items-center mb-3">
      <div class="flex gap-2">
        <button class="px-2 py-1 bg-blue-50 text-blue-700 font-medium rounded hover:bg-blue-100 transition shadow-sm" @click="selectAll">
          Select All
        </button>
        <button class="px-2 py-1 bg-gray-50 text-gray-700 font-medium rounded hover:bg-gray-100 transition shadow-sm" @click="unselectAll">
          Unselect All
        </button>
      ...
      <table class="min-w-full table-auto divide-y divide-gray-200 text-sm">
        <thead class="bg-gray-50 uppercase">
          <tr>
            ...
            <th v-for="h in headers" :key="h" class="px-2 py-2 border border text-left font-semibold text-gray-600 whitespace-nowrap">
              {{ h }}
            </th>
          </tr>
        </thead>
        ...
        <tbody class="divide-y divide-gray-200 bg-white">
          <tr v-for="(bom, index) in filteredBOMs" :key="bom.row_uid" 
            ...
            <td class="px-2 py-2 border text-blue-600 font-medium whitespace-nowrap">{{ bom.bom_no || "No BOM Available" }}</td>
            <td class="px-2 py-2 border whitespace-nowrap">{{ bom.item_code }}</td>
            <td class="px-2 py-2 border whitespace-nowrap">{{ bom.bom_qty }}</td>
            <td class="px-2 py-2 border text-xs whitespace-nowrap"><span class="px-2 py-1 bg-gray-100 rounded-full">{{ bom.bom_type }}</span></td>
            <td class="px-2 py-2 border whitespace-nowrap">{{ bom.customer }}</td>
            <td class="px-2 py-2 border font-bold text-gray-800 whitespace-nowrap">{{ bom.required_for_selected_qty }}</td>
            ...
            <td class="px-2 py-2 border min-w-[180px] whitespace-nowrap">
              <select v-model="bom.selected_workstation" class="w-full text-xs border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500">
            ...
            <td class="px-2 py-2 min-w-[200px] whitespace-nowrap">
              <select v-model="bom.selected_mould" class="w-full text-xs border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500">
    ...
    <div v-if="!filteredBOMs.length && !loading" class="p-10 text-gray-400 text-center border-2 border-dashed border-gray-100 rounded-xl mt-4">
