<template>
	<div class="p-4 shadow-sm border border-gray-200 rounded-lg bg-white relative">
		<p class="text-xs text-gray-500">
			Select Year, Month And Customer For Production Planning
		</p>
		...
		onApplyFilters() {
			...
			if (
				...
				frappe.msgprint({
					...
					message:
						"Please select at least one filter (Year, Month, or Customer) to apply.",
					indicator: "orange",
				});
				return;
			}
			...
			if (
				(this.localFilters.year || this.localFilters.month) &&
				!this.localFilters.customer
			) {
				frappe.msgprint({
					title: "Validation Error",
