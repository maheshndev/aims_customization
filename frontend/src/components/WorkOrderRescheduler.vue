    ...
    <div class="work-orders rounded shadow-sm p-4 border border-gray-200">
        ...
        <div class="overflow-auto rounded-b-2xl">
            <div class="flex items-center gap-3 mb-3">
                ...
                <button class="px-3 py-1 bg-gray-200 text-black rounded hover:bg-gray-300 text-sm" @click="unselectAll">
                    Unselect All
                </button>
                
                <button class="px-3 py-1 bg-green-500 text-white rounded hover:bg-green-600 text-sm flex items-center gap-1"
                    @click="refreshWorkOrders">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004 12a7.961 7.961 0 00-1.565 3.59M18 20v-5h-.582m-15.356-2A8.001 8.001 0 0020 12a7.961 7.961 0 001.565-3.59" />
                    </svg>
                    Refresh
                </button>

                <span class="text-gray-600 text-sm ml-4"> Selected: <span class="font-bold">{{ selectedWorkOrders.length }}</span> / {{ workOrders.length }} </span>
            </div>
            ...
            <table class="min-w-[1600px] table-auto w-full border-collapse text-sm">
                <thead class="bg-gray-100 sticky top-0 uppercase">
                    <tr>
                        ...
                        <th class="border px-3 py-2 text-left whitespace-nowrap">Mat. Transferred</th>
                        <th class="border px-3 py-2 text-right whitespace-nowrap">Disassembled Qty</th>
                ...
                <tbody>
                    <tr v-for="(wo, index) in workOrders" :key="wo.wo_name" 
                        ...
                        <td class="border px-3 py-2 whitespace-nowrap text-right">{{ wo.wo_qty }}</td>
                        <td class="border px-3 py-2 whitespace-nowrap text-right">{{ wo.produced_qty }}</td>
                        ...
                        <td class="border px-3 py-2 whitespace-nowrap text-right">{{ wo.disassembled_qty }}</td>
        ...
        <div v-if="!workOrders.length && !loading" class="text-gray-500 mt-2 p-2 bg-gray-50 rounded">
...
import { api } from "../services/rescheduleApi";

const props = defineProps({
    salesOrders: { 
        type: Array, 
        default: () => [],
        validator: (val) => val.every(item => typeof item === 'string' && item.trim().length > 0)
    },
