<template>
	<div class="blanket-order-items p-2 shadow-md border border-gray-200 rounded-lg">
		<p class="text-xs text-gray-500">
			Select Row and for selected row enter schedule qty in input box in row to generate
			scheduled <b>Sales Order</b>
		</p>
		<div
		...
		>
			<div class="flex flex-wrap items-center gap-2">
				...
				<input
					v-model="searchText"
					type="text"
					placeholder="Search By: Item Name / Item Code / Blanket Order ID "
		...
		<div
			v-if="error"
			class="bg-red-100 text-red-700 px-4 py-2 border border-red-200 rounded mb-4 text-sm mx-4 mt-2"
		>
			{{ error }}
		</div>
				...
				<tbody class="divide-y divide-gray-100">
					...
					>
						...
						<td
							...
							:key="key"
							class="px-1 py-1 border text-gray-800 whitespace-nowrap"
						>
							<template v-if="key === 'schedule_qty'">
								<input
									type="number"
									:min="0"
									:max="item.remaining_bo_qty"
									v-model.number="item.schedule_qty"
									@input="validateScheduleQty(item)"
							...
							<template v-else-if="key === 'bo_name'">
								<a
									...
									:href="`/app/blanket-order/${item[key]}`"
									target="_blank"
									>{{ item[key] }}</a
								>
							</template>
