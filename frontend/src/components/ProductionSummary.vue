<template>
  <div class="space-y-4">
    <button @click="refresh" :disabled="loading"
      class="ml-4 px-3 py-2 text-sm rounded border bg-white hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed m-2">
      🔄 Refresh
    </button>
    ...
    <table class="min-w-[800px] table-auto divide-y divide-gray-200 border rounded-xl shadow-md">
      ...
      <tbody>
        <tr v-for="(r, index) in rows" :key="r.item_code" class="border-b">
          ...
          <td class="border font-medium px-3 py-2 whitespace-nowrap">
            {{ r.item_name }}
          </td>
          <td class="border text-right px-3 py-2 whitespace-nowrap">
            {{ r.order_qty }}
          </td>
          <td class="border text-right text-green-600 px-3 py-2 whitespace-nowrap">
            {{ r.produced_qty }}
          </td>
          <td class="border text-right text-red-500 px-3 py-2 whitespace-nowrap">
            {{ r.balance_qty }}
          </td>
