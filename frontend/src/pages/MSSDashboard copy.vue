  ...
  <div class="p-6 bg-gray-50 min-h-screen font-sans space-y-6">
    ...
    <SectionCard title="Blanket Orders">
      <BlanketOrders :filters="localFilters" v-model:selected="selected.blanketOrders"
        @items-loaded="selected.items = $event" @update:selected="onBlanketOrdersSelected"/>
    </SectionCard>

    <!-- Blanket Order Items -->
    <SectionCard title="Items for Selected BO(s)">
      <BlanketOrderItems ::selected-bo-names="(selected.blanketOrders || []).map(bo => bo.name)"
        v-model:selected="selected.items"  @update:selected="onSelectedBOItems"/>
    </SectionCard>
  ...
  setup() {
    ...
    const onBlanketOrdersSelected = async (selectedBOs) => {
      ...
      try {
        ...
        state.blanketOrderItems = res.data.message || [];
      } catch (e) {
    ...
    const onSelectedBOItems = async (itemsArray) => {
